    ...
    def _get_patient_dict(self):
        ...
        for patient in self.patients_df['case_id']:
            ...
            if isinstance(slide_ids, str):
                ...
                # 加入只取出不大于4个slides的限制，因为对于使用self-attention的模型要考虑显存占用的问题
                slide_ids = slide_ids.values[:4]
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.modality in ['omics', 'snn', 'mlp_per_path']:
        ...
        elif self.modality in ["CLAM_SB", "CLAM_MB", "WiKG", "DSMIL", "TransMIL", "ABMILOmics", "mlp_per_path_wsi", "abmil_wsi", "abmil_wsi_pathways", "deepmisl_wsi", "deepmisl_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways"]:
        ...
        elif self.modality in ["coattn", "coattn_motcat", "MCAT", "MOTCAT", "CMTA", "G_HANet_Surv", "SNNTrans"]:
            
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)

            condition = self.omics_data_dict["rna"]["temp_index"] == case_id
            omic1 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[0]].values))
            omic2 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[1]].values))
            omic3 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[2]].values))
            omic4 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[3]].values))
            omic5 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[4]].values))
            omic6 = torch.squeeze(torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[5]].values))
            ...
            for i in range(self.num_pathways):
                omic_list.append((torch.tensor(self.omics_data_dict["rna"].loc[condition,self.omic_names[i]].values)).squeeze(0))
