...
def _get_custom_exp_code(args):
    ...
    if args.modality in ["ProSurv"]:
        if args.geno_mlp_type != 'SNN':
            param_code += "_genoM_" + str(args.geno_mlp_type)

        if args.sim_loss != 0.2:
            param_code += "_simL_" + str(args.sim_loss)
        
        if args.align_loss != 0.2:
            param_code += "_alignL_" + str(args.align_loss)

        if args.complete_rate != 1.0:
            param_code += "_completeR_" + str(args.complete_rate)
        
        if args.use_align_loss != True:
            param_code += "_noAlignL_"
...
def _get_split_loader(args, split_dataset, training = False, testing = False, weighted = False, batch_size=1):
    ...
    elif args.modality in ["CLAM_SB", "CLAM_MB", "DSMIL", "WiKG", "TransMIL", "ABMILOmics", "abmil_wsi", "abmil_wsi_pathways", "deepmisl_wsi", "deepmisl_wsi_pathways", "mlp_wsi", "transmil_wsi", "transmil_wsi_pathways"]:
        collate_fn = _collate_wsi_omics
    elif args.modality in ["coattn", "coattn_motcat", "MCAT", "MOTCAT", "CMTA", "G_HANet_Surv", "SNNTrans"]:  
        collate_fn = _collate_MCAT
    elif args.modality == "survpath":
         collate_fn = _collate_survpath 
    elif args.modality in ["ProSurv"]:
